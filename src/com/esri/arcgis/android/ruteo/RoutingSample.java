...
package com.esri.arcgis.android.ruteo;
...
import com.esri.core.geometry.Unit.EsriUnit;
...
public class RoutingSample extends Activity implements
	...
	background.setTileModeX(android.graphics.Shader.TileMode.REPEAT);
		...
		ArcGISDynamicMapServiceLayer trafico = new ArcGISDynamicMapServiceLayer(
				"https://traffic.arcgis.com/arcgis/rest/services/World/Traffic/MapServer", null, 
				AccountManager.getInstance().getPortal().getCredentials());
		...
		map.setOnSingleTapListener(new OnSingleTapListener() {
			...
			public void onSingleTap(float x, float y) {
				...
				hiddenSegmentsLayer.updateGraphic(selectedSegmentID,
						segmentShower);
				String direction = ((String) selected.getAttributeValue("texto"));
				double time = ((Double) selected.getAttributeValue("tiempo"))
						.doubleValue();
				double length = ((Double) selected.getAttributeValue("distancia"))
						.doubleValue();
				// Update the label with this direction's information
				String label = String.format("%s%n%.0f minutos (%.0f metros)",
	...
	private void QueryDirections(final Point mLocation, final Point p) {
		...
		dialog = ProgressDialog.show(RoutingSample.this, "Rutas",
				"Calculando Ruta...", true);
		...
		Thread t = new Thread() {
			...
			public void run() {
				try {
					...
					rp.setDirectionsLanguage("es-419");
					
					rp.setDirectionsLengthUnit(DirectionsLengthUnit.METERS);
	...
	void updateUI() {
		...
		FragmentManager fm = getFragmentManager();
		if (fm.findFragmentByTag("Nav") == null) {
			FragmentTransaction ft = fm.beginTransaction();
		...
		for (RouteDirection rd : curRoute.getRoutingDirections()) {
			...
			attribs.put("texto", rd.getText());
			attribs.put("tiempo", Double.valueOf(rd.getMinutes()));
			attribs.put("distancia", Double.valueOf(rd.getLength()));
			
			if(rd.getLength() > 800){
				if((rd.getLength()/1000) < 2){
					curDirections.add(String.format("%s%n%.0f minutos (%.0f kilometro)",
							rd.getText(), rd.getMinutes(), (rd.getLength()/1000)));
				}else{
					curDirections.add(String.format("%s%n%.0f minutos (%.0f kilometros)",
							rd.getText(), rd.getMinutes(), (rd.getLength()/1000)));
				}
				...
				if(rd.getMinutes() < 1){
					Double min = 1.00;
					curDirections.add(String.format("%s%n%.0f minutos (%.0f metros)",
							rd.getText(), min, rd.getLength()));
				}else{
					curDirections.add(String.format("%s%n%.0f minutos (%.0f metros)",
		...
		routeSummary = String.format("%s%n%.0f minutos (%.0f kilometros)",
				"Inicio - Destino", curRoute.getTotalMinutes() ,
				curRoute.getTotalMiles()*1.6);
		directionsLabel.setText(routeSummary);
		...
		curDirections.add(0, "Mi Localización");

		curDirections.remove(curDirections.size() - 1);
		curDirections.add("Destino");
	}
	...
	public void onSegmentSelected(String segment) {
		...
		for (int index : hiddenSegmentsLayer.getGraphicIDs()) {
			...
			if (segment.contains((String) g.getAttributeValue("texto"))) {
